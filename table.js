...
function h(e) {
    ...
    localStorage.setItem('mealPreference', preference);

    // Change the window location after saving the preference
    window.location.href = 'table.html';
}

// On the new page (table.html), retrieve the preference
console.log(window.location.href);

let savedPreference = localStorage.getItem('mealPreference');
if(window.location.href=='http://127.0.0.1:3000/Project/table.html'){

    if (!savedPreference) {
        // Prompt the user for their meal preference
        savedPreference = prompt("Please enter your meal preference (Non-Vegetarian, Vegetarian, Vegan):");
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    switch(savedPreference) {
        ...
        case 'Vegan':
            ...
            break;
        default:
            selectedMeals = []; // or handle the case where no valid preference is found
            break;
    }
    
    // You can now use selectedMeals to generate the meal plan or perform other actions
});
